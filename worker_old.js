...
function runDataValidation(spreadsheetData) {
    ...
    spreadsheetData.forEach(row => {
        ...
        if (!row['PREÇO DE CUSTO'] || row['PREÇO DE CUSTO'].toString().trim() === '') {
            corrections.push({ column: 'PREÇO DE CUSTO', from: '', to: '0' });
            row['PREÇO DE CUSTO'] = '0';
        } else {
            let precoValue = row['PREÇO DE CUSTO'].toString()
                .replace(/[R\$\s]/g, '')
                .replace(/\.(?=\d{3})/g, '');
            
            let tempValue = precoValue.replace(',', '.');
            if (isNaN(parseFloat(tempValue)) || parseFloat(tempValue) < 0) {
                precoValue = '0';
                corrections.push({ column: 'PREÇO DE CUSTO', from: row['PREÇO DE CUSTO'], to: precoValue });
            } else if (row['PREÇO DE CUSTO'].toString() !== precoValue) {
                corrections.push({ column: 'PREÇO DE CUSTO', from: row['PREÇO DE CUSTO'], to: precoValue });
            }
            row['PREÇO DE CUSTO'] = precoValue;
        }
        
        // PREÇO DE VENDA - Validação e correção
        if (!row['PREÇO DE VENDA'] || row['PREÇO DE VENDA'].toString().trim() === '') {
            corrections.push({ column: 'PREÇO DE VENDA', from: '', to: '0' });
            row['PREÇO DE VENDA'] = '0';
        } else {
            let precoValue = row['PREÇO DE VENDA'].toString()
                .replace(/[R\$\s]/g, '')
                .replace(/\.(?=\d{3})/g, '');
            ...
            if (isNaN(parseFloat(tempValue)) || parseFloat(tempValue) < 0) {
                precoValue = '0';
                corrections.push({ column: 'PREÇO DE VENDA', from: row['PREÇO DE VENDA'], to: precoValue });
            } else if (row['PREÇO DE VENDA'].toString() !== precoValue) {
                corrections.push({ column: 'PREÇO DE VENDA', from: row['PREÇO DE VENDA'], to: precoValue });
            }
            row['PREÇO DE VENDA'] = precoValue;
        }
